...
public class heroic_avatar_hardmode_controller extends script.base_script
{
    public heroic_avatar_hardmode_controller()
    {
    }
    public int vosskDied(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        CustomerServiceLog("instance-heroic_avatar_hardmode", "Vossk Defeated in instance (" + self + ") by group_id (" + group + ") at " + realTime);
        CustomerServiceLog("instance-heroic_avatar_hardmode_min", "Group (" + group + ") consists of: ");
        for (int i = 0; i < players.length; ++i)
        {
            String strProfession = skill.getProfessionName(getSkillTemplate(players[i]));
            CustomerServiceLog("instance-heroic_avatar_hardmode", "Group (" + group + ") member " + i + " " + getFirstName(players[i]) + "'s(" + players[i] + ") profession is " + strProfession + ".");
        }
